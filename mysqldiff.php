...
class MysqlDiff
{
    ...
    private function _connect($db)
    {
        ...
        $mysqli = new mysqli($db['host'], $db['user'], $db['pwd'], $db['db'],$db['port']);
        ...
        $mysqli->real_connect($db['host'], $db['user'], $db['pwd'], $db['db'],$db['port']);
        ...
        foreach ($tableArr as $table) {
            ...
            foreach ($maybeChangeDiff as $field => $value) {
                ...
                if (!empty($diffFields)) {
                    ...
                    if ($srcFields[$field]['Key'] == 'PRI' && $destFields[$field]['Key'] != 'PRI') {     
                        $changeSql[] = "ALTER TABLE `$table` DROP PRIMARY KEY;";
                    }


                    if ($destFields[$field]['Key'] == 'PRI' && $srcFields[$field]['Key'] != 'PRI') {
    ...
    public function diffDb()
    {
        ...
        if(empty($this->_src_scheme)){
            exit('_src_scheme is null ');
        }

        $this->_setDbdest();
        $this->_dest_scheme = $this->_getDbScheme();

         if(empty($this->_dest_scheme)){
            exit('_dest_scheme is null ');
        }
...
$src_config = ['host' => '127.0.0.1', 'user' => 'root', 'pwd' => '123456', 'db' => 'xian_qjs_db'];//旧版数据库

$dest_config = ['host' => '127.0.0.1', 'user' => 'root', 'pwd' => '123456', 'db' => 'dest_qjs_db'];//修改过得数据库
